...
const CarouselSection: React.FC<{ images: CarouselImage[] }> = ({ images }) => {
    ...
    return (
        <section className="w-full overflow-hidden relative group rounded-2xl [mask-image:linear-gradient(to_right,transparent,white_10%,white_90%,transparent)]">
            <div 
                className="flex items-center animate-scroll"
                style={{ '--animation-duration': animationDuration } as React.CSSProperties}
            >
                {[...images, ...images].map((img, index) => (
                    <div key={`${img.id}-${index}`} className="flex-shrink-0 mx-4 w-[80vw] sm:w-[45vw] md:w-[30vw] xl:w-[450px]">
                        <img 
                            src={img.url} 
                            alt={`Slide ${index + 1}`} 
  ...
  return (
    <section className="mb-8">
      ...
      {sortedJornadas.length === 0 ? (
        <div className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 border border-slate-700/40 rounded-2xl p-8 text-center">
          <SoccerIcon className="w-16 h-16 mx-auto mb-4 text-gray-500 opacity-60 animate-bounce-slow" />
          <h3 className="text-lg font-bold text-gray-300 mb-2">Las jornadas están por llegar</h3>
          <p className="text-sm text-gray-500 max-w-xs mx-auto leading-relaxed">
            Prepárate para demostrar tus conocimientos. Las próximas jornadas se activarán pronto.
          </p>
          {!currentUser && (
            <p className="mt-6 text-xs text-cyan-400 font-semibold uppercase tracking-wider">
              Regístrate y sé el primero en jugar →
            </p>
          )}
