...
const ConfigurationTab: React.FC<ConfigurationTabProps> = ({ config, setConfig }) => {
  ...
  const toggleSectionVisibility = (sectionKey: AppConfig['sectionsOrder'][0]) => {
      const currentHidden = config.theme.hiddenSections || [];
      const newHidden = currentHidden.includes(sectionKey)
          ? currentHidden.filter(key => key !== sectionKey)
            ...
            {config.sectionsOrder.map((sectionKey, index) => {
                ...
                >
                    <div className="flex items-center gap-4 cursor-grab">
                        ...
                        <span className={`font-medium ${isHidden ? 'text-gray-400 line-through' : 'text-white'}`}>{sectionNames[sectionKey]}</span>
                    </div>
                    <button 
        ...
        <details className="bg-gray-800 p-4 rounded-lg">
            ...
            <div className="mt-4 grid md:grid-cols-1 gap-6">
                <div className="bg-gray-700/50 p-4 rounded-lg space-y-4">
                    <h3 className="font-bold text-xl text-cyan-400 mb-2">Premio "Gordito"</h3>
                    ...
                    <div>
                        <label className="block mb-1 text-sm font-medium">Fondo de El Gordito (Valor dinámico)</label>
                        <input type="number" value={config.botinAmount || 0} onChange={e => handleValueChange('botinAmount', Number(e.target.value))} className="w-full bg-gray-600 p-2 rounded" step="10"/>
                    </div>
                    <div>
