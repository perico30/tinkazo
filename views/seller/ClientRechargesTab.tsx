...
const ClientRechargesTab: React.FC<ClientRechargesTabProps> = ({ currentUser, config, onProcessClientRecharge }) => {
    ...
    const clientRequests = useMemo(() => {
        return config.rechargeRequests
            .filter(r => r.requesterRole === 'client' && myClientIds.includes(r.userId))
    ...
    return (
        <>
            ...
            <div className="bg-gray-800 p-4 rounded-lg">
                <h2 className="font-semibold text-lg mb-4">Historial de Recargas de Clientes ({clientRequests.length})</h2>
                {/* ═══ MOBILE CARDS (< md) ═══ */}
                <div className="md:hidden space-y-3">
                        ...
                        return (
                            <div key={req.id} className="bg-slate-800/50 border border-slate-700/50 p-4 rounded-xl">
                                ...
                                <div className="grid grid-cols-2 gap-2 text-xs bg-slate-900/40 rounded-lg p-2.5 mb-3">
                                    ...
                                    <div>
                                        <span className="text-gray-500 uppercase tracking-wider text-[10px]">Comprobante</span>
                                        <button onClick={() => setViewingProof(req.proofOfPaymentUrl || null)} className="text-cyan-400 text-xs font-semibold disabled:text-gray-500" disabled={!req.proofOfPaymentUrl}>Ver</button>
                                    </div>
                                </div>
                                {req.status === 'pending' ? (
                                    <div className="flex gap-2">
                                        <button onClick={() => onProcessClientRecharge(req.id, 'approve', currentUser.id)} className="flex-1 py-2 rounded-lg bg-green-600 text-white font-bold text-xs hover:bg-green-500 transition">Aprobar</button>
                                        <button onClick={() => onProcessClientRecharge(req.id, 'reject', currentUser.id)} className="flex-1 py-2 rounded-lg bg-red-600 text-white font-bold text-xs hover:bg-red-500 transition">Rechazar</button>
                                    </div>
                                ) : (
                    ...
                    {clientRequests.length === 0 && <p className="text-center text-gray-500 py-8">No hay solicitudes de clientes.</p>}
                </div>
                                ...
                                return (
                                    <tr key={req.id} className="border-b border-slate-800 hover:bg-slate-800/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <button onClick={() => setViewingProof(req.proofOfPaymentUrl || null)} className="text-cyan-400 hover:underline text-xs font-semibold disabled:text-gray-500 disabled:no-underline" disabled={!req.proofOfPaymentUrl}>Ver</button>
                                        </td>
                                        <td className="px-6 py-4">
                                        ...
                                        <td className="px-6 py-4 text-right whitespace-nowrap space-x-2">
                                            {req.status === 'pending' ? (
                                                <>
                                                    <button onClick={() => onProcessClientRecharge(req.id, 'approve', currentUser.id)} className="px-3 py-1 rounded bg-green-600 text-white font-bold text-xs hover:bg-green-500">Aprobar</button>
                                                    <button onClick={() => onProcessClientRecharge(req.id, 'reject', currentUser.id)} className="px-3 py-1 rounded bg-red-600 text-white font-bold text-xs hover:bg-red-500">Rechazar</button>
                                                </>
                                            ) : (
